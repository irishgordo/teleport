...
func TestGenerateCerts(t *testing.T) {
	...
	t.Run("ImpersonateAllow", func(t *testing.T) {
		...
		require.NoError(t, err)
		superImpersonator, err := test.CreateUser(srv.Auth(), "superimpersonator", superImpersonatorRole)
		require.NoError(t, err)
		...
		require.NoError(t, err)
		impersonator, err := test.CreateUser(srv.Auth(), "impersonator", role)
		require.NoError(t, err)

		iUser := User(impersonator.GetName())
		iUser.TTL = time.Hour
...
func newTLSServer(t *testing.T) *TLSServer {
	return newTLSServerWithConfig(t, AuthServerConfig{
		Dir:     t.TempDir(),
		Clock:   clockwork.NewFakeClock(),
...
func newTLSServerWithConfig(t *testing.T, config AuthServerConfig) *TLSServer {
