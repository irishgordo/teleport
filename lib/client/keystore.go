...
func initKeysDir(dirPath string) (string, error) {
	dirPath = FullProfilePath(dirPath)
...
func (fs *FSLocalKeyStore) AddKey(key *Key) error {
	...
	if err := fs.writeBytes(key.Pub, inProxyHostDir(key.Username+fileExtPub)); err != nil {
		return trace.Wrap(err)
	}
	if err := fs.writeBytes(key.TLSCert, inProxyHostDir(key.Username+fileExtTLSCert)); err != nil {
		return trace.Wrap(err)
	}

	// Store per-cluster key data.
	if err := fs.writeBytes(key.Cert, inProxyHostDir(key.Username+sshDirSuffix, key.ClusterName+fileExtSSHCert)); err != nil {
		return trace.Wrap(err)
	}
	...
	for kubeCluster, cert := range key.KubeTLSCerts {
		...
		path := inProxyHostDir(key.Username+kubeDirSuffix, key.ClusterName, kubeCluster+fileExtTLSCert)
		if err := fs.writeBytes(cert, path); err != nil {
			return trace.Wrap(err)
		}
	}
	for db, cert := range key.DBTLSCerts {
		path := inProxyHostDir(key.Username+dbDirSuffix, key.ClusterName, filepath.Clean(db)+fileExtTLSCert)
		if err := fs.writeBytes(cert, path); err != nil {
			return trace.Wrap(err)
		}
	}
	for app, cert := range key.AppTLSCerts {
		path := inProxyHostDir(key.Username+appDirSuffix, key.ClusterName, filepath.Clean(app)+fileExtTLSCert)
		if err := fs.writeBytes(cert, path); err != nil {
			return trace.Wrap(err)
		}
	}
...
func (fs *FSLocalKeyStore) DeleteKey(idx KeyIndex) error {
	files := []string{
		...
		fs.inSessionKeyDir(idx.ProxyHost, idx.Username+fileExtPub),
		fs.inSessionKeyDir(idx.ProxyHost, idx.Username+fileExtTLSCert),
	}
...
func (fs *FSLocalKeyStore) GetKey(idx KeyIndex, opts ...CertOption) (*Key, error) {
	...
	pub, err := ioutil.ReadFile(fs.inSessionKeyDir(idx.ProxyHost, idx.Username+fileExtPub))
	if err != nil {
		fs.log.Error(err)
		return nil, trace.ConvertSystemError(err)
	}
	tlsCertFile := fs.inSessionKeyDir(idx.ProxyHost, idx.Username+fileExtTLSCert)
...
func (fs *FSLocalKeyStore) updateKeyWithCerts(o CertOption, key *Key) error {
	...
	if info.IsDir() {
		...
		for _, certFile := range certFiles {
			...
			name := strings.TrimSuffix(certFile.Name(), fileExtTLSCert)
...
func (o WithSSHCerts) relativeCertPath(idx KeyIndex) string {
	...
	if idx.ClusterName != "" {
		components = append(components, idx.ClusterName+fileExtSSHCert)
	}
...
func (o WithDBCerts) relativeCertPath(idx KeyIndex) string {
	...
	if idx.ClusterName != "" {
		components = append(components, idx.ClusterName)
		if o.dbName != "" {
			components = append(components, o.dbName+fileExtTLSCert)
		}
	}
	return filepath.Join(components...)
}
...
func (o WithAppCerts) relativeCertPath(idx KeyIndex) string {
	...
	if idx.ClusterName != "" {
		components = append(components, idx.ClusterName)
		if o.appName != "" {
			components = append(components, o.appName+fileExtTLSCert)
		}
	}
	return filepath.Join(components...)
}
...
func (fs *fsLocalNonSessionKeyStore) inSessionKeyDir(path ...string) string {
	return filepath.Join(fs.KeyDir, sessionKeyDir, filepath.Join(path...))
}
...
func (fs *fsLocalNonSessionKeyStore) SaveTrustedCerts(proxyHost string, cas []auth.TrustedCerts) (retErr error) {
	...
	certsFile := fs.inSessionKeyDir(proxyHost, fileNameTLSCerts)
...
func (fs *fsLocalNonSessionKeyStore) GetTrustedCertsPEM(proxyHost string) ([][]byte, error) {
	data, err := ioutil.ReadFile(fs.inSessionKeyDir(proxyHost, fileNameTLSCerts))
	if err != nil {
		return nil, trace.ConvertSystemError(err)
	}
	var blocks [][]byte
